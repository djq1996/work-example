...
//常用日期设置函数
function rangeDate(dateId, dateIds) {
	var start = {
		elem: dateId,
	...
	laydate(start);
	//如果有默认值则触发choose事件
	if($(start.elem).val().length > 0){
		start.choose($(start.elem).val());
	}
	laydate(end);
	//如果有默认值则触发choose事件
	if($(end.elem).val().length > 0){
		end.choose($(end.elem).val())
	}
}



function radioDate(dateId) {
	laydate({
	  elem: dateId,
...
function radioDateForValidate(dateId,formId) {
	laydate({
	  elem: '#'+dateId,
	  format: 'YYYY-MM-DD', // 分隔符可以任意定义，该例子表示只显示年月
	  festival: true, //显示节日
	  choose: function(datas){ //选择日期完毕的回调
          ...
          .updateStatus(dateId, 'NOT_VALIDATED',null)
          .validateField(dateId);
	  }
	});
}

function rangeDateForValidate2(dateId, dateIds, formId) {
	var start = {
		elem: '#'+dateId,
		//format: 'YYYY/MM/DD hh:mm:ss',
		//min: laydate.now(), //设定最小日期为当前日期
	...
	var end = {
		...
		choose: function(datas) {
			start.max = datas; //结束日选好后，重置开始日的最大日期
			/*$(formId).data('bootstrapValidator')
	          .updateStatus(dateIds, 'NOT_VALIDATED',null)
	          .validateField(dateIds);*/
		}
	};
	laydate(start);
	//如果有默认值则触发choose事件
...
function rangeDateForValidate(dateId, dateIds, formId) {
	var start = {
		elem: '#'+dateId,
		//format: 'YYYY/MM/DD hh:mm:ss',
		//min: laydate.now(), //设定最小日期为当前日期
		...
		choose: function(datas) {
			...
			$(formId)  
		    .bootstrapValidator('updateStatus', dateId, 'NOT_VALIDATED', null)
		    .bootstrapValidator('validateField', dateId);
		}
	};
	var end = {
		elem: '#'+dateIds,
		...
		choose: function(datas) {
			start.max = datas; //结束日选好后，重置开始日的最大日期
			$(formId)  
		    .bootstrapValidator('updateStatus', dateIds, 'NOT_VALIDATED', null)
		    .bootstrapValidator('validateField', dateIds);
		}
	};
	laydate(start);
	//如果有默认值则触发choose事件
