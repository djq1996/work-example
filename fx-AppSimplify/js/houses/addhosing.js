...
//						};
		...
		getHouseInfo() {
			...
			mui.ajax(baseUrl + 'API_House/HouseData?HouseID=' + vm.$data.HouseID, {
				type: 'post',
				success: function(data, type, status) {
					//					console.log('43行获取提交信息'+JSON.stringify(vm.$data.AddHouseinfo))
					if(data.succ && data.results != null) {

						console.log('48行获取提交信息' + JSON.stringify(data))
						$("input[name=intention]").val(data.results.Housename + ' ' + data.results.Builder + '#'+self.UnitAsName +'#'+ data.results.fang);
...
mui.plusReady(function() {
	...
	if(self.RoomId != undefined) {
		...
		if(self.HouseId != 0) {
			...
			console.log(self.name + ' ' + self.Builder + '#' + self.UnitAsName + '#' + self.fang);
			vm.$data.AddHouseinfo.PerId = parseInt(localStorage.getItem('userId'));
			vm.$data.AddHouseinfo.PerName = localStorage.getItem('userName');
...
$('#subBtn').on('tap', function() {
	/*判断重复转出，或者第一次转出*/
	if($("input[name=sell_rent]:checked").length == 2||checked){
		/*获取手机产权人信息*/
		var PhoneList = [],
			carList = [];
		...
		console.log('添加修改房源' + JSON.stringify(vm.$data.AddHouseinfo));
		$(this).attr({
			"disabled": "disabled"
		});
		vm.AddHouseInfo();
...
$('.add-linkman .add').on('tap', function() {
	$('.importLink').append('<li class="mui-table-view-cell box"><a class="mui-navigate-right mui-input-row area" href="javascript:;">' +
		...
		'<label>产权人关系：</label>' +
		'<input type="text" placeholder="请选择" class="" name="IsLookUser" readonly="readonly" value="" />' +
		'</a>' +
